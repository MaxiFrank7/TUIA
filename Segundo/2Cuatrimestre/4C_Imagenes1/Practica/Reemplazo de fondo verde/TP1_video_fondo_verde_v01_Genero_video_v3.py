...
out = cv2.VideoWriter('output_v3.avi', fourcc, 20.0, (video_person["width"], video_person["height"]))
...
FILTER_PB_SIGMA = 0
SHADOW_COLOR_HSV = np.array([59, 245, 175])    
SHADOW_DELTA = np.array([10.0, 70.0, 70.0])
SHADOW_SE_SIZE = 3
SHADOW_BLUR_SE_SIZE = 3
SHADOW_PROF = 40
k = 0
while(cap_person.isOpened()):
    ...
    # --- Paso 5: Genero sombra ------------------------------------------------------------
    lower_limmit = np.clip(SHADOW_COLOR_HSV - SHADOW_DELTA, 0, 255)
    upper_limmit = np.clip(SHADOW_COLOR_HSV + SHADOW_DELTA, 0, 255)
    mask_shadow = cv2.inRange(frame_hsv, lower_limmit, upper_limmit)
    mask_shadow = cv2.morphologyEx(mask_shadow, cv2.MORPH_OPEN, np.ones((SHADOW_SE_SIZE, SHADOW_SE_SIZE)))
    mask_shadow = cv2.morphologyEx(mask_shadow, cv2.MORPH_CLOSE, np.ones((SHADOW_SE_SIZE, SHADOW_SE_SIZE)))
    mask_shadow_blur = cv2.GaussianBlur(mask_shadow/255, (SHADOW_BLUR_SE_SIZE, SHADOW_BLUR_SE_SIZE), 0)
    for n in range(3):
        res[:, :, n] = np.clip(res[:, :, n] - mask_shadow_blur*SHADOW_PROF, 0, 255)

    # --- Paso 6: Grabo frame en video -------------------------------------------
    out.write(res)
